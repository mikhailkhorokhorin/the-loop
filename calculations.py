...
Fc = x = y = v_x = v_y = a_x = a_y = h = c_x = c_y = v = air_density = drag = lift = 0
...
# Функция, возвращающая угол во время 1-ого этапа выполнения "мертвой" петли.
def angle_while_loop_I() -> float:
    return 90 * (T - 41.5) / 5


# Функция, возвращающая угол во время 2-ого этапа выполнения "мертвой" петли.
def angle_while_loop_II() -> float:
    return 90 + 90 * (T - 46.5) / 5


# Функция, возвращающая угол во время 3-ого этапа выполнения "мертвой" петли.
def angle_while_loop_III() -> float:
    return 180 + 90 * (T - 51.5) / 5


# Функция, возвращающая угол во время 4-ого этапа выполнения "мертвой" петли.
...
def calculate(alpha: float, flag: int) -> None:
    ...
    Fc = m * v ** 2 / loop_radius - Fg * np.sin(
        alpha * np.pi / 180)  # Центробежная сила (в Н). Необходимое условие выполнения петли - ее положительное значение (не равна нулю).
